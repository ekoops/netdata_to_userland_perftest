 ...
 * 	to user space, perf_buffer events should be preferred.
 *
 * Returns
 ...
 * bpf_perf_event_read
 *
 * 	Read the value of a perf_buffer event counter. This helper relies on a
 * 	*map* of type **BPF_MAP_TYPE_PERF_EVENT_ARRAY**. The nature of
 * 	the perf_buffer event counter is selected when *map* is updated with
 * 	perf_buffer event file descriptors. The *map* is an array whose size
 ...
 *
 * 	Note that before Linux 4.13, only hardware perf_buffer event can be
 * 	retrieved.
 *
 ...
 *
 * Returns
 * 	The value of the perf_buffer event counter read from the map, or a
 ...
 *
 * 	Write raw *data* blob into a special BPF perf_buffer event held by
 * 	*map* of type **BPF_MAP_TYPE_PERF_EVENT_ARRAY**. This perf_buffer
 ...
 * 	call **perf_event_open**\ () on the perf_buffer event (either for
 ...
 *
 * 	Read the value of a perf_buffer event counter, and store it into *buf*
 * 	of size *buf_size*. This helper relies on a *map* of type
 * 	**BPF_MAP_TYPE_PERF_EVENT_ARRAY**. The nature of the perf_buffer event
 * 	counter is selected when *map* is updated with perf_buffer event file
 ...
 * 	more PMU based perf_buffer events opened than available counters,
 ...
 * 	enabled and running times are accumulated since the perf_buffer event
 * 	open. To achieve scaling factor between two invocations of an
 * 	eBPF program, users can use CPU id as the key (which is
 * 	typical for perf_buffer array usage model) to remember the previous
 ...
 *
 * 	For en eBPF program attached to a perf_buffer event, retrieve the
 ...
 *
 * 	For an eBPF program attached to a perf_buffer event, retrieve the
