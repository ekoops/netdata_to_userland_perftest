...
# load and attach tracepoint programs on the interface and start to read from ring buffer in background
taskset $((1 << READER_PIN_CORE_NUM)) "$OUTPUT_DIR"/tracepoint/tracepoint "$SERVER_PID" "$BUFFER_PAGES_NUM" "$READER_TRAFFIC_FILTER" &
PERF_READER_PID=$!
